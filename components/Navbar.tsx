...
export function Navbar() {
  ...
  // Mock login for demo
  const handleLogin = () => {
    const email = prompt("Enter email (demo):", "dev@example.com");
    if (email) {
      const name = prompt("Enter name:", "Developer");
      if (name) login(email, name);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <Link href="/dashboard" className="text-sm font-medium hover:text-neutral-600 transition-colors">
              Dashboard
            </Link>
        ...
        {isMenuOpen && (
          ...
          >
            <div className="px-4 py-6 space-y-4 flex flex-col">
              ...
              <Link href="/dashboard" onClick={() => setIsMenuOpen(false)} className="text-lg font-medium">
                Dashboard
              </Link>
              <div className="pt-4 border-t border-neutral-100">
                {user ? (
                  <div className="flex flex-col gap-4">
                    <span className="text-sm text-neutral-500">Signed in as {user.name}</span>
                    <button 
